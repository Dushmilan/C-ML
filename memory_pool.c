...
void* pool_alloc(MemoryPool* pool, size_t bytes){
    if(!pool ||!pool->pool|| bytes == 0) return NULL;

    const size_t align = 16;
    size_t aligned = (pool->used + (align - 1)) & ~(align - 1);

    if(aligned + bytes > pool->pool_size) return NULL; //out of memory
...
MemoryPool* get_pool(void){
    if(!g_pool) g_pool = pool_create(1u<<30); // 1 GiB pool for all tensors
    return g_pool;
}
